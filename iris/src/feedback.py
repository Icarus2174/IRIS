    ...
    if runtime_event.type in ("orbital_link_confidence_low", "optical_path_unavailable", "topology_instability_detected", "safe_mode_requested"):
        if runtime_event.type == "orbital_link_confidence_low":
            ...
            if isinstance(conf, (int, float)):
                rationale["runtime_confidence"] = float(conf)
        ...
        # If a numeric confidence is supplied, only enforce packet downgrade when it is below the
        # current IR threshold (keeps event payload meaningful and avoids unconditional downgrades).
        if runtime_event.type == "orbital_link_confidence_low":
            conf = runtime_event.details.get("confidence")
            if isinstance(conf, (int, float)):
                min_conf = float(ir.space_constraints.get("min_link_confidence_for_orbit", 0.6))
                rationale["min_confidence_threshold"] = min_conf
                prefer_packet = float(conf) < min_conf
